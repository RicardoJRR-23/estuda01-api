...
describe('PUT /studyModules/:studyModuleId', () => {
  let access_token;
  let user_id;
  ...
  describe('Success cases', () => {
    it('Should be able to update the entire study module instance', async () => {
      ...
      await request(app)
        .put(`/studyModules/${study_module_id}`)
        .set('authorization', `Bearer ${access_token}`)
        .send(payload)
        .expect(204);
  ...
  describe('Error cases', () => {
    ...
    it('Should return status code 400 when the user does not provide the field topics', async () => {
      const payload = {
        title: 'Matemática',
        description: 'Ramo do conhecimento'
      };
      ...
      expect(response.body).toMatchObject({
        error: 'Campo "topics" está em falta.'
      });
    });
    ...
    it('Should return 404 if the id provided does not belong to an existing study module', async () => {
      ...
      const response = await request(app)
        .put(`/studyModules/${new mongoose.Types.ObjectId()}`)
        .set('authorization', `Bearer ${access_token}`)
        .send(payload)
        .expect(404);
